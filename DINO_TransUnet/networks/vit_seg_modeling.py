...
class Block(nn.Module):
    ...
    def load_from_dino(self, weights, n_block):
        
        with torch.no_grad():
            query_weight, key_weight, value_weight =  weights[f'blocks.{n_block}.attn.qkv.weight'].view(3,768,768)
            out_weight =  weights[f'blocks.{n_block}.attn.proj.weight']

            query_bias, key_bias, value_bias =  weights[f'blocks.{n_block}.attn.qkv.bias'].view(3,768)
...
class VisionTransformer(nn.Module):
    ...
    def load_from_dino(self, res_weights, weights):
        with torch.no_grad():

            self.transformer.embeddings.patch_embeddings.weight.copy_(np2th(res_weights["embedding/kernel"], conv=True))
            self.transformer.embeddings.patch_embeddings.bias.copy_(np2th(res_weights["embedding/bias"]))

            self.transformer.encoder.encoder_norm.weight.copy_(weights['norm.weight'])
            self.transformer.encoder.encoder_norm.bias.copy_(weights['norm.bias'])

            posemb = np2th(res_weights["Transformer/posembed_input/pos_embedding"])
            ...
            for bname, block in self.transformer.encoder.named_children():
                for uname, unit in block.named_children():
                    unit.load_from_dino(weights, n_block=uname)

            if self.transformer.embeddings.hybrid:
                self.transformer.embeddings.hybrid_model.root.conv.weight.copy_(np2th(res_weights["conv_root/kernel"], conv=True))
